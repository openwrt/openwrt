...
CShoreWrt takes a different approach to building a firmware; downloading, patching
and compiling everything from scratch, including the cross compiler. To put it
in simpler terms, CShoreWrt does not contain any executables or even sources, it is an
...
just apply to the kernel, but to anything included with CShoreWrt -- It is this one
simple understated concept which is what allows CShoreWrt to stay on the bleeding edge
with the latest compilers, latest kernels and latest applications.

So let's take a look at CShoreWrt and see how this all works.
...
\texttt{package} is for exactly that -- packages. In an CShoreWrt firmware, almost everything
...
\subsubsection{Building CShoreWrt}

While the CShoreWrt build environment was intended mostly for developers, it also has to be
simple enough that an inexperienced end user can easily build his or her own customized firmware.

Running the command "\texttt{make menuconfig}" will bring up CShoreWrt's configuration menu
...
CShoreWrt will only display a high level overview of the compile process and not each individual
command.
...
One of the things that we've attempted to do with CShoreWrt's template system is make it
incredibly easy to port software to CShoreWrt. If you look at a typical package directory
in CShoreWrt you'll find several things:
...
The files directory is also optional and typicall contains package specific startup scripts or default configuration files that can be used out of the box with CShoreWrt.
...
another keyword of \texttt{@GNU} for grabbing GNU source releases. If any of the above mentionned download source fails, the CShoreWrt mirrors will be used as source.
...
multiple packages from a single source. CShoreWrt works under the assumption of one
...
The CShoreWrt distribution makes the distinction between two kind of kernel modules, those coming along with the mainline kernel, and the others available as a separate project. We will see later that a common template is used for both of them.
...
CShoreWrt provides a means of building images for multiple configurations
