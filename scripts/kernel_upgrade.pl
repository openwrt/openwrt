...
sub new()
{
	...
	# fast-import's "commit" command *requires* a ref; as such we create a
	# "DISCARD_HEAD", which Git really should implement itself...
	# We also need to account for worktrees.
	if(-d '.git') {
		unlink('.git/DISCARD_HEAD');
	} elsif(open(my $g, '<', '.git')) {
		$_=<$g>;
		chomp;
		unlink("$_/DISCARD_HEAD") if($_=~s,^gitdir:\s/,/,);
	}
...
unless($author->[0]);
	...
	$self->putcmd("commit DISCARD_HEAD");
