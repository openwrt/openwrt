...
# no chance to add CAMEO tags at the default locations. This would leave 
# the kernel partition without a tag. So we must find suitable space.
# 
# Location for original kernel partition is at the end of the uImage header. 
# We will reuse the last bytes of the IH_NAME field. This is the tricky 
# part because we have the header CRC and the CAMEO checksum that must match
# the whole header. uImage header CRC checksums all data expect the CRC data
# itself. The same applies for CAMEO checksum. It checksums all preceding 
# data except itself. Changing one of both results in a change of the other.
...
# can be found by U-Boot. The CAMEO checksum calculation must start at
# position 0x180000 (see DLINK_KERNEL_PART_SIZE) and includes the "second"
# half of the "split" kernel uImage. 
...
IMAGE_NAM = bytes([0x4f, 0x70, 0x65, 0x6e, 0x57, 0x72, 0x74, 0x00, 0x00, 0x00, 0x00, 0x00])
CRC_00 = bytes([0x00] * 4)
CRC_FF = bytes([0xff] * 4)
...
def checksum_header(buf):
    buf[UIMAGE_NAM_OFF:UIMAGE_NAM_END] = IMAGE_NAM + CAMEO_TAG
    buf[UIMAGE_CRC_OFF:UIMAGE_CRC_END] = CRC_FF
    buf[UIMAGE_SUM_OFF:UIMAGE_SUM_END] = cameosum(buf[0:UIMAGE_NAM_END])
...
if args.uimage_file.tell() <= args.rootfs_start:
    raise ValueError(f"uImage must be larger than " + str(args.rootfs_start) + " bytes")
