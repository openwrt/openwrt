...
def parse_args():
    ...
    parser.add_argument("-f", "--source-format", required=True, choices=['apk', 'opkg'],
                        help="Required source format of input: 'apk' or 'opkg'")
    parser.add_argument("-m", "--manifest", default=False,
                        help="File includes the packages to be included in the output")

    # fmt: on
    args = parser.parse_args()
    return args

def get_apk_sbom(text: str, installed: list) -> dict:
...
def get_opkg_sbom(text: str, installed: list) -> dict:
    components: list = []
    ...
    for chunk in chunks:
        ...
        if 'license' in package:
            licenses: list = []
            for license in package["license"].split('OR'):
                licenses.append({"license": {"name": license}})
            element.update({"licenses": licenses})
...
if __name__ == "__main__":
    ...
    timestamp: str = datetime.datetime.now(datetime.UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
    cyclonedx: dict = {
        "bomFormat": "CycloneDX",
        "specVersion": "1.4",
