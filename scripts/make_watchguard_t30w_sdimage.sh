...
# Create p1 placeholder (must be >= 1024 blocks for modern e2fsprogs)
...
# Create p3 boot partition (U-Boot loads uImage_t30, rename for compatibility)
"$hostbin/mkfs.ext2" -q -F "$workdir/p3.img" 32768
"$hostbin/debugfs" -w -R "write $workdir/uImage /uImage" "$workdir/p3.img"
"$hostbin/debugfs" -w -R "write $workdir/uImage /uImage_t30" "$workdir/p3.img"
"$hostbin/debugfs" -w -R "write $workdir/t30.dtb /t30.dtb" "$workdir/p3.img"

# Write p3
dd if="$workdir/p3.img" of="$output_raw" bs=512 seek=$((p3_offset / 512)) conv=notrunc status=none
...
echo "Done: $output_gz"
echo "Layout: p1=1MB(placeholder), p2=64MB(squashfs), p3=32MB(boot), p4=2GB(rootfs_data,expandable)"
