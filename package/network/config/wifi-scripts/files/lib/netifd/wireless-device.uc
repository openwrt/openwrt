...
{
	if (wdev.script_timeout)
		wdev.script_timeout.cancel();
	delete wdev.script_timeout;
	delete wdev.script_task;
	cb(wdev);
}

function run_handler_timeout(wdev, cb)
{
	wdev.script_task.cancel();
...
function run_handler(wdev, op, cb)
{
	...
	wdev.script_task = netifd.process({
	...
	wdev.script_timeout = uloop.timer(DEFAULT_SCRIPT_TIMEOUT, () => run_handler_timeout(wdev, cb));
}
...
function __wdev_proc_check(wdev, proc)
{
	...
	warn(`${wdev.name}: process ${proc.exe}(${proc.pid}) no longer active\n`);
	wdev.teardown();
	return true;
}
