...
static int run_backend(char *const argv[], int timeout)
{
	...
	setpgid(pid, pid);	/* races the child's own setpgid(); either may win */
	close(fds[1]);
	alarm(timeout);
	f = fdopen(fds[0], "r");
	if (f) {
	...
	/* the backend answered or the alarm expired: kill the whole group so a
	 * stuck gcom cannot keep holding the modem's tty, then reap the child */
	kill(-pid, SIGKILL);
