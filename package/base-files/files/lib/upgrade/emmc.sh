...
emmc_upgrade_tar() {
	...
	tar tf "$tar_file" ${board_dir}/dtb 1>/dev/null 2>/dev/null && has_dtb=1
	...
	[ "$has_dtb" = 1 -a "$EMMC_DTB_DEV" ] &&
		export EMMC_DTB_BLOCKS=$(($(tar xf "$tar_file" ${board_dir}/dtb -O | dd of="$EMMC_DTB_DEV" bs=512 2>&1 | grep "records out" | cut -d' ' -f1)))
