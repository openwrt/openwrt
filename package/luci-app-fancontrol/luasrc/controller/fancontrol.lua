...
function index()
    entry({"admin", "system", "fancontrol"}, call("action_fancontrol"), _("Fan Control"), 90).dependent = true
...
function action_api()
    ...
    if action == "get_config" then
        write_json({
            ...
            fan_file = uci_get(uci, "fan_file", "/sys/devices/platform/pwm-fan/hwmon/hwmon0/pwm1"),
            fan_rpm_file = uci_get(uci, "fan_rpm_file", "/sys/class/hwmon/hwmon0/fan1_input"),
            max_speed = uci_get(uci, "max_speed", "255"),
    ...
    elseif action == "get_status" then
        ...
        local fan_file = uci_get(uci, "fan_file", "/sys/devices/platform/pwm-fan/hwmon/hwmon0/pwm1")
        local fan_rpm_file = uci_get(uci, "fan_rpm_file", "/sys/class/hwmon/hwmon0/fan1_input")
        local max_speed = tonumber(uci_get(uci, "max_speed", "255"))
    ...
    elseif action == "restart" then
        require("luci.sys").call("/etc/init.d/fancontrol restart >/dev/null 2>&1")
        write_json({ result = "restarted" })
