...
static int emc2305_identify(struct device *dev)
{
	...
	case EMC2305:
		data->pwm_num = 5;
                dev_info(dev, "%d detected emc2305\n", ret);
		break;
	case EMC2303:
		data->pwm_num = 3;
                dev_info(dev, "%d detected emc2303\n", ret);
		break;
	case EMC2302:
		data->pwm_num = 2;
                dev_info(dev, "%d detected emc2302\n", ret);
		break;
	case EMC2301:
		data->pwm_num = 1;
                dev_info(dev, "%d detected emc2301\n", ret);
		break;
	default:
                dev_info(dev, "%d not detected emc23xx\n", ret);
		return -ENODEV;
	}
