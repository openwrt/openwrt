...
static int aw9106_parse_led_children(struct aw9106_chip *chip)
{
	...
	device_for_each_child_node_scoped(chip->dev, child) {
		...
		if (i >= AW9106_MAX_CHANNELS) {
			dev_err(chip->dev,
				"too many led child nodes (max %d), ignoring %pfw\n",
		...
		if (ret || channel >= AW9106_MAX_CHANNELS) {
			dev_err(chip->dev,
				"led node %pfw missing/invalid reg (0..%d)\n",
		...
		fwnode_property_read_string(child, "default-state", &default_state);
		if (default_state && !strcmp(default_state, "on"))
			led->cdev.brightness = led->cdev.max_brightness;
		else
			led->cdev.brightness = LED_OFF;

		/* fwnode drives the classdev name: it is composed from the
		...
		ret = devm_led_classdev_register_ext(chip->dev, &led->cdev,
						      &init_data);
		if (ret) {
			dev_err(chip->dev, "failed to register led %pfw: %d\n",
				child, ret);
			continue;
		}
