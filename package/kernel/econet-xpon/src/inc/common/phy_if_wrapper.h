...
/* PORT 6.18: timer_setup() callback signature */
void check_sync_result(struct timer_list *t);
void try_set_phy_mode(struct timer_list *t);

static inline void schedule_phy_mode_auto_detection(void)
{
    gpPhyData->detect_status = XPON_PHY_SYNC_STATUS_NOT_START ;
    XPON_START_TIMER(gpPhyData->delay_start_detect_timer, 3000);	/* was .data = 3000 ms */
}
...
/* The ECNT hook layer is a no-op stub in this port (foundation-shim: __ECNT_HOOK
 * and ecnt_register_hook do nothing), so going through it silently drops the call.
 * The digital-PHY dispatcher IS present + hook-registered (phy_init.c). Call it
 * directly so XPON_PHY_SET/GET actually reach the PHY (mode config, CDR, etc.).
 * Unhandled cmd_ids return PHY_NO_API; do not panic (avoids crashing on optional cmds). */
extern int pon_phy_api_dispatch(struct ecnt_data *in_data);
