...
#define IO_SPHYREG(reg, val)				iowrite32(val, (void __iomem *)(reg)) /* SetReg((uint)reg, val) */
	...
	#endif
	/* PORT FIX (2026-07-02): digital GPON PHY is MMIO at 0xbfaf0000+sub on this EN7528,
	 * not an external A60928 on SIF/I2C. Match the stock (reads 0xbfaf0130/0x5e0 raw).
	 * See phy/inc/phy_reg_util.h for the full rationale. */
	#define IO_GPHYREG(sub_addr)				ioread32((void __iomem *)(0xbfaf0000u + (unsigned int)(sub_addr)))
...
/* PORT: re-enabled (guarded) -- xpon_set_qos() compares against bare
 * ENABLE/DISABLE; no header in this build defines them as enum members. */
#ifndef ENABLE
#define ENABLE	1
#endif
#ifndef DISABLE
