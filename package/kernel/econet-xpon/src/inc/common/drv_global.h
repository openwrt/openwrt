...
/* PORT 6.18: struct timer_list lost its .data field, which used to carry the
 * re-arm interval (ms) consumed here.  Pass the interval explicitly now. */
#define XPON_START_TIMER(timer, ms)		{ mod_timer(&(timer), (jiffies + msecs_to_jiffies(ms))) ; }
