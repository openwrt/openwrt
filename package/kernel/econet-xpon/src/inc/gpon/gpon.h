...
typedef struct {
	...
	/* FIX 2 (2026-07-04): cache Extended_Burst_Length T3 preamble so it can be applied on O3/O4
	 * entry — the ext-burst PLOAM may arrive in O2 (dropped by the O3-only guard) but the MAC fires
	 * SN autonomously the instant it enters O3, so the ~119B T3 preamble must already be set or the
	 * OLT's burst receiver can't AGC/CDR-lock the SN burst (= clone O3 stall). */
	unchar					o3T3Cache ;
	unchar					o5T3Cache ;
	unchar					extBurstValid ;
	unchar					dvtPcpCheck ;
	uint					dvtPcpCounter ;
	uint					dvtPcpCheckErr ;
