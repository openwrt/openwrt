...
#include <linux/foe_hook.h>	/* PORT: ra_sw_nat_hook_* extern decls */
...
void start_omci_oam_monitor(void)
{
	...
	/* PORT 6.18: timer_list lost .data; the re-arm interval is passed to
	 * XPON_START_TIMER below from gpMonitorParams->timer_interval. */
...
/* PORT 6.18: timer_setup() callback sig (legacy ->data interval unused here) */
void xpon_count_rx_pkt(struct timer_list *t)
{
...
int xpon_omci_oam_monitor_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	else if (!strcmp("interval", argv[1]))
	{
        ...
        /* PORT 6.18: .data removed; interval is applied at XPON_START_TIMER. */
        gpMonitorParams->max_timeout_cnt =
...
void omci_oam_monitor_init(Omci_Oam_Monitor_t * monitor_data_ptr)
{
	...
	/* PORT 6.18: init_timer()+.data+.function -> timer_setup(); the interval
	 * (default 1000 ms, else gpMonitorParams->timer_interval) is applied at
	 * each XPON_START_TIMER arm site. */
	timer_setup(&gpMonitor->xponCntRxPkt, xpon_count_rx_pkt, 0);
