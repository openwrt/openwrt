...
/*
 * fe_reg_write() is the MTK/EcoNet frame-engine register write helper that the
 * vendor SDK exports from raeth.  hwNAT/FE is OFF for the compile/port
 * milestone, and the real wiring to the econet_eth QDMA/FE driver is a later
 * step, so stub it to a no-op here.   TODO EN7528: wire to econet_eth FE.
 */
#ifndef ECONET_XPON_HAVE_FE_REG
#define ECONET_XPON_HAVE_FE_REG
static inline void fe_reg_write(unsigned int reg, unsigned int val)
{
	(void)reg;
	(void)val;
}
