...
#endif
/*
 * foe_hook.h defines struct port_info, which gpon_wan.h (pulled in via
 * xpon_netif.h below) uses by pointer in prototypes, and which the TX path
 * uses by value.  The vendor only included it under TCSUPPORT_RA_HWNAT; on a
 * hwNAT-off build that left struct port_info prototype-scoped, tripping 6.18
 * "conflicting / incompatible pointer type".  Include it unconditionally and
 * BEFORE the driver headers so it is a complete, file-scope type everywhere.
 */
...
/*
 * KERNEL_3_18_21 is (mis)defined inside the TCSUPPORT_XPON_LED block further
 * down, so on a build without the LED feature (e.g. EN7528) it is never
 * defined and every "#if KERNEL_3_18_21" version gate evaluates the wrong way
 * (3-arg alloc_netdev, ndo_set_multicast_list, NETIF_F_NO_CSUM ...).  Define it
 * unconditionally here.  (The dead in-block copy redefines to the same tokens,
 * which C permits silently.)   TODO EN7528: drop the in-block definition.
 */
#ifndef KERNEL_3_18_21
#define KERNEL_3_18_21		(LINUX_VERSION_CODE >= KERNEL_VERSION(3,18,19))
#endif

/*
 * Lowercase fixed-width SDK typedefs (uint8/uint16/uint32) came from a vendor
 * platform header not present in the GPL drop (only the UINT* spellings live in
 * drv_types.h, and the kernel provides plain 'uint').  Define them locally.
 * TODO: move into a shared compat header -- uint16/uint32/uint8 are also used
 * by the gpon/xmcs sources.
 */
#ifndef ECONET_XPON_HAVE_LC_UINTS
#define ECONET_XPON_HAVE_LC_UINTS
...
/* 6.18 timer callback trampoline: timer_setup() expects (struct timer_list *) */
static void econet_xpon_pwan_net_timer_cb(struct timer_list *t)
{
	(void)t;
	pwan_net_timer(0);
}
...
#endif
	{
		timer_setup(&gpWanPriv->pwan_timer, econet_xpon_pwan_net_timer_cb, 0);
		gpWanPriv->pwan_timer.expires = jiffies + msecs_to_jiffies(250);
...
#define GOTO_DROP(n) do {XPON_DROP_PRINT; goto drop##n; }while(0)
    ...
    if(SOFT_LOOPBACK_MODE(pNetPriv)) {
        ...
        /* dev->last_rx was removed in 6.x; the core tracks rx timing now */
        netif_rx(skb) ;
        goto post_qdma_tx;
...
if((PWAN_IF_DATA == pNetPriv->netIdx ) && sw_pktqosEnqueue){
    ...
    if (pNetPriv->netIdx == PWAN_IF_OMCI) {
        /* ★ OMCI TX BRIDGE (2026-07-09): route the daemon's OMCI reply through the REAL
         * en75_qdma seam (macSend -> en75_qdma_xmit, egress GDM2, tag = gem_port).
         * The old qdma_transmit_packet() path (below, EN7521-gated) goes through the k6.18
         * no-op __ECNT_HOOK stub and FAULTS the kernel on EN7528 when a reply is sent.
         * macSend has NULL guards + linearizes; validated by the /proc tx-test.
         * Strip the daemon's 0x88b5 OMCI-over-eth header (if any) so the GEM carries raw OMCI. */
        if (skb->len >= 14 && skb->data[12] == 0x88 && skb->data[13] == 0xb5)
            skb_pull(skb, 14);
        /* ★ OMCI RESPONSE FIXUP (2026-07-09): the minimal-MIB daemon answers the OLT's
         * provisioning-verify GETs with values that make the OLT refuse to send the service
         * Create: ONU-data(class 2) MIB-data-sync GET returns result=0x02 (should be 0x00),
         * and ONU2-G(class 257) reports OMCC-version 0xA0 while the cloned DSNW subscriber is
         * provisioned as 0xA1. Rewrite these two GET responses in-place (raw OMCI at data[0],
         * no MIC here — HW adds it downstream) so the OLT advances to Create. */
        if (skb->len >= 12) {
            unsigned char *o = skb->data;
            if ((o[2] & 0x3f) == 0x29 && o[3] == 0x0a) {   /* GET response, baseline DevID */
                unsigned int cls  = (o[4] << 8) | o[5];
                unsigned int mask = (o[9] << 8) | o[10];
                if (cls == 2 && o[8] != 0x00) {
                    o[8] = 0x00;
                    printk(KERN_DEBUG "OMCIDBG: fixup ONU-data GET result -> 00\n");
                } else if (cls == 257 && o[8] == 0x00 && mask == 0x4000 && o[11] != 0xA1) {
                    o[11] = 0xA1;
                    printk(KERN_DEBUG "OMCIDBG: fixup ONU2-G OMCC-ver -> A1\n");
                }
            }
        }
        XPON_SKB(skb)->gem_port = GPON_OMCC_ID;
        printk(KERN_DEBUG "OMCIDBG: TX OMCI len=%u gem=%u -> en75_qdma\n", skb->len, (uint)GPON_OMCC_ID);
        macSend(WAN2LAN_CH_ID, skb);	/* consumes skb */
        ret = 0;
    } else
#ifdef TCSUPPORT_CPU_EN7521
...
post_qdma_tx:
    ...
    netif_trans_update(dev) ;	/* 6.x: dev->trans_start moved to the txq */
    return ret ;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,26)
		napi_complete(napi);	/* 6.18: __napi_complete() was removed */
#else
...
int pwan_create_net_interface(uint idx)
{
	...
	/* 6.18: dev_addr is const; write it through the accessor (copies ETH_ALEN) */
	eth_hw_addr_set(gpWanPriv->pPonNetDev[idx], mac_addr) ;
	...
	/* 6.18: netif_napi_add() lost its weight arg; use the _weight variant */
...
/* 6.18 timer callback trampoline: timer_setup() expects (struct timer_list *) */
static void econet_xpon_txdrop_timer_cb(struct timer_list *t)
{
	(void)t;
	pwan_tx_drop_timer(0);
}
...
int pwan_init(void)
{
	int i, ret ;
#if defined(TCSUPPORT_CPU_PERFORMANCE_TEST)
	struct compat_proc_priv *ftpTestProc=NULL;
#endif
#if defined(TCSUPPORT_CT_PON_GDV20) || defined(TCSUPPORT_IS_FH_PON)
	struct compat_proc_priv *tr143Proc=NULL;
#endif
	struct compat_proc_priv *ponProc;
  ...
  gpWanPriv->devCfg.flags.isQosUp = 1;
	...
	gpWanPriv->devCfg.flags.isTxDropOmcc = 0;
	/* 6.18: init_timer()/.function/.data removed; the 5000 ms period that used
	 * to live in .data is now the TXDROP_TIMER_MS constant in gpon_wan.c. */
	timer_setup(&gpWanPriv->txDropTimer, econet_xpon_txdrop_timer_cb, 0);
