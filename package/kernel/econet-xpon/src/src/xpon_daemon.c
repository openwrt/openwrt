...
static void handle_xpon_daemon_job_get_phy_params(void)
{
    ...
    extern void *gpPhyPriv;   /* type-agnostic; only need the NULL check here */

	PON_MSG(MSG_TRACE, "%s\n", __FUNCTION__) ;

    /* PORT: on module (re)load the daemon can run this poll before the phy private
     * data is allocated -> the transceiver reads deref gpPhyPriv (NULL) -> Oops.
     * Skip until the phy is up; the daemon re-schedules the poll later. */
    if (!gpPhyPriv)
        return;
...
static void handle_xpon_daemon_job_fe_reset(void)
{
    ...
    if(PHY_GPON_CONFIG == gpPhyData->working_mode){
        ...
        XPON_START_TIMER(gpPonSysData->gsw_p6_rate_timer, 5000);	/* PORT 6.18: was .data=5000 */
    }
