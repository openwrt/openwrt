...
void check_sync_result(struct timer_list *t)	/* PORT 6.18: timer_setup() callback sig */
{
    ulong flags;
    ...
    {
        XPON_START_TIMER(gpPhyData->check_sync_timer, 1000);	/* was .data = 1000 ms */
        return;
    }
    ...
    if( XPON_PHY_GET(PON_GET_PHY_IS_SYNC) ){
        ...
        try_set_phy_mode(NULL);	/* PORT 6.18: direct (non-timer) call */
    }
}

void try_set_phy_mode(struct timer_list *t)	/* PORT 6.18: timer_setup() callback sig */
{
