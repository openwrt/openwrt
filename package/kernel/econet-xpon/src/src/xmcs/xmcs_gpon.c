...
/* ---- 6.18 port compat (xmcs group) -------------------------------------
 * (1) These gponDev* helpers are defined in gpon_dev.c but the vendor never
 *     declared them in gpon_dev.h, so calling them here is an implicit
 *     declaration (-Werror). Provide local externs; they resolve at link time
 *     from gpon_dev.o.
 *     TODO: move these prototypes into inc/gpon/gpon_dev.h (gpon group).
 */
extern int gponDevGetSniffMode(GPON_DEV_SNIFFER_MODE_T *sniffer);
extern int gponDevGetTxSyncOffset(unchar *tx_sync);
extern int gponDevClearGemPortCounter(ushort gemPortId);

/* (2) gpon_dvt.c is intentionally DROPPED from the build (DVT/bring-up only).
 *     Stub its entry points to no-ops so the GPON cmd dispatcher still links.
 *     TODO EN7528: if DVT is ever needed, re-add gpon_dvt.c to the Kbuild and
 *     delete these stubs.
 */
static inline int gpon_dvt_eqd_adjustment(uint neqEqd)      { (void)neqEqd; return 0; }
static inline int gpon_dvt_aes_key_switch_by_sw(void)       { return 0; }
static inline int gpon_dvt_sw_reset(void)                   { return 0; }
static inline int gpon_dvt_sw_resync(void)                  { return 0; }
static inline int gpon_dvt_tod(uint newTime)                { (void)newTime; return 0; }
/* ------------------------------------------------------------------------ */
...
static int xmcs_set_act_timer(struct XMCS_GponActTimer_S *pActTimer) 
{
	...
	/* TODO 6.18 timer port: struct timer_list lost its .data member (removed
	 * 4.15). The TO1/TO2 timeout period is the canonical copy in
	 * gponCfg.to1Timer/to2Timer; the ported timer callbacks
	 * (gpon_act_to{1,2}_timer_expires) must read it via timer_container_of()
	 * instead of timer->data. The legacy .data write is therefore dropped. */
...
static int xmcs_set_act_timer_to1(uint to1Timer) 
{
	...
	/* TODO 6.18 timer port: .data removed; period kept in gponCfg.to1Timer
	 * (read it via timer_container_of() in the ported callback). */
...
static int xmcs_set_act_timer_to2(uint to2Timer) 
{
	...
	/* TODO 6.18 timer port: .data removed; period kept in gponCfg.to2Timer
	 * (read it via timer_container_of() in the ported callback). */
