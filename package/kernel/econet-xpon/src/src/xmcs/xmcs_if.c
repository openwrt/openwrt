...
/* 6.18 port compat (xmcs group): xpon_stop_timer() is defined in xpondrv.c
 * but the vendor never declared it in any header, so calling it here is an
 * implicit declaration (-Werror). Provide a local extern; it links from
 * xpondrv.o.
 * TODO: move this prototype into a shared header (xpon_daemon.h / xpondrv.h). */
...
#ifdef TCSUPPORT_CPU_EN7521
	//todo: not implement yet
	return 0 ;	/* PORT 6.18 / TODO EN7528: PSE PCP config not implemented */
#else
...
int xpon_set_qdma_qos(int enable)
{
	...
	return 0 ;	/* PORT 6.18: vendor fell off end; -Werror=return-type */
}
#else
