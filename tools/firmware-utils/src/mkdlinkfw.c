...
static int find_auh_headers(char *buf)
{
	...
	int buf_size = inspect_info.file_size;
		...
		if (!memcmp(tmp_buf, AUH_MAGIC, 3)) {
			if (((struct auh_header *)tmp_buf)->header_checksum ==
			    (uint16_t) ~ jboot_checksum(0, (uint16_t *) tmp_buf,
							AUH_SIZE - 2)) {
				uint16_t checksum = 0;
				printf("Find proper AUH header at: 0x%X!\n",
				       tmp_buf - buf);
...
static int check_stag_header(char *buf, struct stag_header *header)
{
	...
	if (header->tag_checksum ==
	    (uint16_t) ~ jboot_checksum(0, (uint16_t *) header,
					STAG_SIZE - 2)) {
		uint16_t checksum = 0;
		printf("Find proper STAG header at: 0x%X!\n",
		       (char *)header - buf);
		checksum =
...
static int check_sch2_header(char *buf, struct sch2_header *header)
{
	...
	if (crc32_tmp == crc32(0, header, header->header_length)) {
		uint32_t crc32_val;
		printf("Find proper SCH2 header at: 0x%X!\n",
		       (char *)header - buf);

		crc32_val =
		    crc32(0, (char *)header + header->header_length,
			  header->image_len);
...
static int inspect_fw(void)
{
	...
	sch2_header_kernel =
	    (struct sch2_header *)(buf + AUH_SIZE + STAG_SIZE);
...
int fill_sch2(struct sch2_header *header, char *kernel_ptr, char *rootfs_ptr)
{
	...
	header->image_crc32 = crc32(0, kernel_ptr, kernel_info.file_size);
	...
	header->rootfs_crc32 = crc32(0, rootfs_ptr, rootfs_info.file_size);
	header->header_crc32 = 0;
	header->header_length = SCH2_SIZE;
	header->cmd_line_length = 0;

	header->header_crc32 = crc32(0, header, header->header_length);
...
int build_fw(void)
{
	...
	if (add_jffs2_eof)
		writelen = pad_jffs2(buf, writelen, firmware_size);
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt(argc, argv, "f:i:jhk:m:o:r:s:S");
		if (c == -1)
			break;
