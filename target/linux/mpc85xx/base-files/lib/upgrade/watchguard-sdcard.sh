...
watchguard_t30_do_upgrade() {
	...
	rootfs="$(sed -n 's/.*[ \t]root=\([^ \t]*\).*/\1/p' /proc/cmdline)"
	if [ -z "$rootfs" ]; then
	...
	get_image "$tar_file" | tar -O -xf - sysupgrade-watchguard_firebox-t30/kernel 2>/dev/null | dd of="$bootfs" bs=4M conv=fsync status=none || {
		echo "Error: Failed to extract kernel."
		return 1
	}

	# 2. Extract rootfs to rootfs partition (p2)
	get_image "$tar_file" | tar -O -xf - sysupgrade-watchguard_firebox-t30/root 2>/dev/null | dd of="$rootfs" bs=4M conv=fsync status=progress || {
