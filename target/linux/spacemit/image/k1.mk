...
ifeq ($(DUMP),)
# Mark each image with its own MBR signature, so runtime can determine which
# one was initially written.
# See also target/linux/spacemit/k1/base-files/etc/board.d/03_sysinfo
SIG_SD:=0x44434453
SIG_EMMC:=0x434d4d45
SIG_OTHER:=0x5248544f

# Use seperate partition GUIDs for each image, so the correct rootfs is used
# even if one boots e.g. of eMMC with a SD card of the same build still inserted
GUID_SD:=$(shell echo $(SOURCE_DATE_EPOCH)$(LINUX_VERMAGIC)sdcard | $(MKHASH) md5 | sed -E 's/(.{8})(.{4})(.{4})(.{4})(.{10})../\1-\2-\3-\4-\500/')
...
define Build/k1-gpt-sd
	rm -f $@.gpt-sd
	ptgen -o $@.gpt-sd -g -l 1024 -D -H -S "${SIG_SD}" -G "${GUID_SD}" \
...
define Build/k1-gpt-emmc
	rm -f $@.gpt-emmc
	ptgen -o $@.gpt-emmc -g -l 1024 -D -H -S "${SIG_EMMC}" -G "${GUID_EMMC}" \
...
define Build/k1-gpt-other
	rm -f $@.gpt-other
	ptgen -o $@.gpt-other -g -l 1024 -D -H -S "${SIG_OTHER}" -G "${GUID_OTHER}" \
...
define Device/generic
  ...
  DEVICE_DTS := \
	...
	k1-orangepi-rv2
  DEVICE_DTS_MULTI := \
	$(KDIR)/image-k1-bananapi-cm6-io.dtb:bananapi,bpi-cm6-io \
  ...
  IMAGE/sdcard.img.gz := \
	...
	pad-to $(ROOTFS_OFFSET_SD)k | append-rootfs | pad-extra 256k |\
	libdeflate-gzip | append-metadata
  IMAGE/emmc.img.gz := \
	k1-gpt-emmc |\
	pad-to 192k | k1-uboot |\
	pad-to 3264k | k1-bootfs |\
	pad-to $(ROOTFS_OFFSET_EMMC)k | append-rootfs | pad-extra 256k |\
	libdeflate-gzip | append-metadata
  IMAGE/other.img.gz := \
	k1-gpt-other |\
	pad-to 128k | k1-bootfs |\
	pad-to $(ROOTFS_OFFSET_OTHER)k | append-rootfs | pad-extra 256k |\
	libdeflate-gzip | append-metadata
endef
