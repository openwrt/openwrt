...
static u64 disable_polling(int port)
{
	...
	default:
		pr_warn("%s not implemented for this SoC\n", __func__);
		break;
	}
...
static void rtl8214fc_on_off(struct phy_device *phydev, bool on)
{
	/* fiber ports */
...
static int rtl8221b_set_eee(struct phy_device *phydev, struct ethtool_eee *e)
{
	int port = phydev->mdio.addr;
	u64 poll_state;
	bool an_enabled;
	...
	// RestartAutoNegotiation
	if (an_enabled) {
		pr_info("%s actually restarting autoneg\n", __func__);
		val = phy_read_mmd(phydev, MDIO_MMD_VEND2, 0xA400);
		val |= BIT(9);
		phy_write_mmd(phydev, MDIO_MMD_VEND2, 0xA400, 0x9200);
	}
 ...
 */
static int rtl9300_read_status(struct phy_device *phydev, u32 sds_num)
{
	u32 status, latch_status, mode;
...
/*
 * Read the link and speed status of the internal SerDes of the RTL9300
 * TODO: This function does not work for XSGMII links
 */
...
static int rtl93xx_read_status(struct phy_device *phydev)
{
	...
	if (soc_info.family == RTL9300_FAMILY_ID)
		return rtl9300_read_status(phydev, sds_num);
...
void rtl9310_sds_tx_config(int sds, phy_interface_t phy_if)
{
	int impedance, pre_amp, main_amp, post_amp, pre_en, post_en;
	int asds, even_sds;
	...
	even_sds = asds - (asds % 2);

	rtl9310_sds_field_w( even_sds, 0x2e, 0x8, 10, 7, impedance);
	rtl9310_sds_field_w( asds, 0x2e, 0x1, 4, 0, pre_amp);
	rtl9310_sds_field_w( asds, 0x2e, 0x1, 9, 5, main_amp);
	rtl9310_sds_field_w( asds, 0x2e, 0x1, 14, 10, post_amp);
	rtl9310_sds_field_w( asds, 0x2e, 0x0, 1, 1, pre_en);
	rtl9310_sds_field_w( asds, 0x2e, 0x0, 0, 0, post_en);
}
...
/*
 * Configuration routine for the RTL9300 SerDes and internal PHYs which
 * are used for SFP+ ports
 */
...
int rtl9310_sds_sym_err_get(int sds, phy_interface_t mode)
{
	u32 val, val2;
	int asds = rtl931x_get_analog_sds(sds);
	int dsds = (sds < 2)?sds:(sds - 1) * 2;
	int evensds = sds - (sds % 2);
	int evenasds = rtl931x_get_analog_sds(evensds);
	int blk_err, ch_0, latch_blk_lock, latch_hiber, ber;

	switch (mode) {
	case PHY_INTERFACE_MODE_10GBASER:
		ch_0 = blk_err = rtl9310_sds_field_r(asds, 0x5, 1, 7, 0);
		val = rtl9310_sds_field_r(asds, 0x5, 1, 15, 14);
		latch_blk_lock = (val >> 1) & 0x1;
		latch_hiber = (val & 0x1);

		rtl931x_write_sds_phy(evenasds, 0x1f, 0x02, 0x35);
		val = rtl9310_sds_field_r(evenasds, 0x1f, 0x14, 3, 2);
		ber = ((val >> (sds % 2)) & 0x1);
		break;

	case PHY_INTERFACE_MODE_1000BASEX:
		rtl9310_sds_field_w(dsds, 0x1, 24, 2, 0, 0x0);

		val = rtl9310_sds_field_r(dsds, 0x1, 3, 15, 8);
		val2 = rtl9310_sds_field_r(dsds, 0x1, 2, 15, 0);
		ch_0 = (val << 16) | val2;

		rtl9310_sds_field_w(dsds, 0x1, 3, 15, 8, 0x0);
		rtl9310_sds_field_w(dsds, 0x1, 2, 15, 0, 0x0);
		break;
...
static void rtl931x_symerr_clear(u32 sds, phy_interface_t mode)
{
	...
	case PHY_INTERFACE_MODE_XGMII:
		...
		break;
	default:
		rtl9310_sds_sym_err_get(sds, mode);
		break;
	}
...
/* Configures the RTL931x SerDes "fiber" modes which include also SGMII
 * and HiSGMII
 * When the SerDes is in these modes, the SerDes Mode Control register
 * is used to switch the SerDes "off" and the actual configuration of
 * the mode is done through internal registers of the SerDes
 * */
void rtl931x_sds_fiber_mode_set(u32 sds, phy_interface_t mode)
{
	...
	pr_info("%s serdes %d, requested mode is %s\n", __func__, sds, phy_modes(mode));
	/* clear symbol error count before changing mode */
	rtl931x_symerr_clear(sds, mode);

	val = 0x9F;
	pos = sds & 0x3;
	sw_w32_mask(0xff << pos, val << pos, RTL931X_SERDES_MODE_CTRL + 4 * (sds >> 2));
	...
	case PHY_INTERFACE_MODE_NA:
		...
		break;
	default:
		pr_err("%s invalid SerDes mode %x, switching SerDes off\n", __func__, mode);
		val = 0x3f;
	}
...
void rtl931x_fiber_adapt(int sds, phy_interface_t mode)
{
	int asds = rtl931x_get_analog_sds(sds);
	int ch_0;

	rtl9310_sds_field_w(asds, 0x2e, 0xe, 13, 11, 2);

	rtl9310_sds_field_w(asds, 0x2e, 0xc, 14, 10, 0);

	pr_info("%s starting SDS %d RX calibration...\n", __func__, sds);
	rtl931x_init_leq_dfe(sds);
	rtl9310_sds_field_w(asds, 0x2e, 0xf, 6, 6, 0);
	rtl9310_sds_field_w(asds, 0x2e, 0xf, 12, 12, 0);

	// Set DFE_TAP0
	rtl9310_sds_field_w(asds, 0x2e, 0x1c, 5, 5, 0);
	rtl9310_sds_field_w(asds, 0x2e, 0x1c, 4, 0, 31);
	rtl9310_sds_field_w(asds, 0x2e, 0xf, 6, 6, 1);

	// Set DFE_VTH
	rtl9310_sds_field_w(asds, 0x2f, 0x12, 11, 4, 0);
	rtl9310_sds_field_w(asds, 0x2e, 0xf, 12, 12, 1);

	rtl931x_sds_rx_rst(sds);

	rtl9310_sds_field_w(asds, 0x2e, 0xf, 7, 7, 0);
	rtl9310_sds_field_w(asds, 0x2e, 0xf, 8, 8, 0);
	rtl9310_sds_field_w(asds, 0x2e, 0xf, 9, 9, 0);
	rtl9310_sds_field_w(asds, 0x2e, 0xf, 10, 10, 0);

	rtl931x_symerr_clear(sds, mode);
	mdelay(150);

	ch_0 = rtl9310_sds_sym_err_get(sds, mode);
	pr_info("%s got symbol errors %d\n", __func__, ch_0);
}
...
void rtl931x_leq_adapt(int sds)
{
	...
	rtl9310_sds_field_w(asds, 0x2e, 0xd, 6, 0, 0x0);
	rtl9310_sds_field_w(asds, 0x2e, 0xd, 13, 13, 0x0);

	// DFE disable
	rtl9310_sds_field_w(asds, 0x2A, 0xf, 12, 6, 0x7f);

	rtl9310_sds_field_w(asds, 0x2e, 0xd, 7, 7, 1);
	rtl931x_sds_rx_rst(sds);
	mdelay(10);

	rtl9310_sds_field_w(asds, 0x2e, 0xd, 7, 7, 0);
	mdelay(100);
}
...
{
	struct device *dev = &phydev->mdio.dev;
	struct rtl83xx_shared_private *shared;
	int addr = phydev->mdio.addr;
	u32 val;
...
static int rtl93xx_serdes_probe(struct phy_device *phydev)
{
	int addr = phydev->mdio.addr;
	...
	if (addr >= RTL931X_CPU_PORT)
		return -ENODEV;

	phydev_info(phydev, "Detected internal RTL9310 Serdes on port %d\n", addr);
	return 0;
}
...
static struct phy_driver rtl83xx_phy_driver[] = {
	...
	},
	{
		...
		.features	= PHY_GBIT_FIBRE_FEATURES,
		.flags		= PHY_HAS_REALTEK_PAGES,
		.probe		= rtl93xx_serdes_probe,
		.config_init	= rtl93xx_configure_serdes,
		.suspend	= genphy_suspend,
		.resume		= genphy_resume,
		.set_loopback	= genphy_loopback,
