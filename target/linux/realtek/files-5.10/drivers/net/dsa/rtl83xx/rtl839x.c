...
static void rtl839x_led_init(struct rtl838x_switch_priv *priv)
{
	...
	sw_w32(0, RTL839X_LED_SET_CTRL(0));
	sw_w32(0, RTL839X_LED_SET_CTRL(1));
	for (i = 0; i < 4; i++) {
		pos = (i % 2)?15:0;
		...
		sw_w32_mask(0x7fff << pos, v << pos, RTL839X_LED_SET_CTRL(i / 2));
	}
	...
	if (of_property_read_u32(node, "leds_per_port", &v) == 0) {
		...
		if (v > 0x3) {
			pr_err("Maximum value of leds_per_port property is 3, using that.\n");
			v = 0x3;
		}
