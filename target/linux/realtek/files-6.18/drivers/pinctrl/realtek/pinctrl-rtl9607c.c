...
struct rtl9607c_pinctrl {
	 ...
	 */
	const struct rtl9607c_functions *pin_states[RTL9607C_GPIO_PINS_NUM];
};
...
static const struct rtl9607c_functions groups_and_functions[] = {
			    ...
			    unsigned int group)
{
	...
	const struct rtl9607c_functions *pgroup = &groups_and_functions[func];
	int i;
...
{
	struct rtl9607c_pinctrl *pctrl = pinctrl_dev_get_drvdata(pctldev);
	const struct rtl9607c_functions *pgroup = pctrl->pin_states[pin];
