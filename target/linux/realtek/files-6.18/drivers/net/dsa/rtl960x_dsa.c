				     ...
				     struct netlink_ext_ack *extack)
{
	...
	/*
	 * DSA reuses this callback for the host (CPU-port) VLAN notifier, and
	 * it keeps BRIDGE_VLAN_INFO_UNTAGGED from the bridge VLAN. Force the
	 * CPU port to be a *tagged* member of every VLAN that has user
	 * members, so VLAN traffic destined to the CPU (the bridge / routing)
	 * is forwarded there with its tag intact. The entries that untag
	 * toward the CPU are the reserved standalone VIDs, rejected above,
	 * and the transparent default VLAN, which a bridge VLAN replaces here.
	 */
