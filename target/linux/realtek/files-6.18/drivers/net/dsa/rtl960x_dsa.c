 ...
 * exposes 5 internal GPHY addresses for the user facing ports; an efuse mask
 * identifies which are disabled. It also has 2 HSGMII interfaces that can be
 * connected to external PHYs, 2 interfaces for CPU ports and one SerDes
 * interface to operate with external PON transceiver.
 */
...
static int rtl960x_switch_bringup(struct rtl960x_dsa *priv)
{
	...
	ret = regmap_read(priv->map, SWCORE_EFUSE_BOND_RSLT, &disport);
	if (ret)
		return ret;
