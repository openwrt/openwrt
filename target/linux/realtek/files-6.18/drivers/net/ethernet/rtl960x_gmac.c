...
static void rtl960x_gmac_set_rx_mode(struct net_device *dev)
{
	...
	gmac_w32(g, GMAC_MAR0, mar);
	gmac_w32(g, GMAC_MAR4, mar);
	gmac_w32(g, GMAC_RCR, rcr);
}
...
static int rtl960x_gmac_alloc_rings(struct rtl960x_gmac *g)
{
	...
	g->rx_ring = dma_alloc_coherent(g->dma_dev,
					RX_RING_SIZE * sizeof(*g->rx_ring),
					&g->rx_ring_dma, GFP_KERNEL);
	if (!g->rx_ring)
		return -ENOMEM;

	g->tx_ring = dma_alloc_coherent(g->dma_dev,
...
err_free_rx_bufs:
	...
	dma_free_coherent(g->dma_dev, TX_RING_SIZE * sizeof(*g->tx_ring),
			  g->tx_ring, g->tx_ring_dma);
	g->tx_ring = NULL;
err_free_rx_ring:
	dma_free_coherent(g->dma_dev, RX_RING_SIZE * sizeof(*g->rx_ring),
...
static void rtl960x_gmac_init_hw(struct rtl960x_gmac *g)
{
	...
	gmac_w32(g, GMAC_CPUTAGCR,
		  RTL9607C_CPUTAG_CONFIG | CPUTAGCR_EN_RX);
	gmac_w32(g, GMAC_CPUTAG1CR, CPUTAG1CR_SID);
	...
	gmac_w8(g, GMAC_RX_FC_ASSERT_THRES, RX_FC_ASSERT_THRES_VAL);
	gmac_w8(g, GMAC_RX_FC_DEASSERT_THRES, RX_FC_DEASSERT_THRES_VAL);
	...
	/* Enable the datapath. */
	gmac_w32(g, GMAC_IO_CMD1, RTL9607C_IO_CMD1_CONFIG);
	gmac_w32(g, GMAC_IO_CMD, RTL9607C_IO_CMD_CONFIG |
		  IO_CMD_RX_ENABLE | IO_CMD_TX_ENABLE);

	/* Clear and unmask RX + TX-completion interrupts. */
	gmac_w16(g, GMAC_ISR, 0xffff);
	gmac_w32(g, GMAC_ISR1, 0xffffffff);
	gmac_w16(g, GMAC_IMR, INTR_NAPI);
}

static void rtl960x_gmac_mask_irqs(struct rtl960x_gmac *g)
{
	gmac_w16(g, GMAC_IMR, gmac_r16(g, GMAC_IMR) & ~INTR_NAPI);
}
...
static irqreturn_t rtl960x_gmac_isr(int irq, void *dev_id)
{
	...
	status = gmac_r16(g, GMAC_ISR) & INTR_NAPI;

	if (!status)
		return IRQ_NONE;

	gmac_w16(g, GMAC_ISR, status);
