...
/* Realtek RTL9607C ethernet driver */
...
{
	regmap_write(g->map32, GMAC_IO_CMD, 0);
	regmap_write(g->map32, GMAC_IO_CMD1, 0);
	regmap_write(g->map16, GMAC_IMR, 0);
	regmap_write(g->map32, GMAC_IMR0, 0);
	regmap_write(g->map16, GMAC_ISR, 0xffff);
	regmap_write(g->map32, GMAC_ISR1, 0xffffffff);
	usleep_range(10, 20);
}

static void rtl960x_gmac_set_hwaddr(struct rtl960x_gmac *g)
{
	const u8 *a = g->dev->dev_addr;

	regmap_write(g->map32, GMAC_IDR0,
		     (a[0] << 24) | (a[1] << 16) | (a[2] << 8) | a[3]);
	regmap_write(g->map32, GMAC_IDR4, (a[4] << 24) | (a[5] << 16));
}

static void rtl960x_gmac_set_rx_mode(struct net_device *dev)
{
	u32 rcr = RCR_ACCEPT_BROADCAST | RCR_ACCEPT_MYPHYS;
	struct rtl960x_gmac *g = netdev_priv(dev);
	u32 mar = 0;
	...
	regmap_write(g->map32, GMAC_MAR0, mar);
	regmap_write(g->map32, GMAC_MAR4, mar);
	regmap_write(g->map32, GMAC_RCR, rcr);
}
...
static void rtl960x_gmac_free_rx(struct rtl960x_gmac *g)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		if (!g->rx_skb[i])
			continue;
		dma_unmap_single(g->dma_dev, g->rx_buf_dma[i], RX_BUF_SIZE,
				 DMA_FROM_DEVICE);
...
static int rtl960x_gmac_init_rings(struct rtl960x_gmac *g)
{
	int i;
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		...
		skb = netdev_alloc_skb(g->dev, RX_BUF_SIZE);
		if (!skb)
			goto err_free_rx_bufs;

		dma = dma_map_single(g->dma_dev, skb->data, RX_BUF_SIZE,
				     DMA_FROM_DEVICE);
		if (dma_mapping_error(g->dma_dev, dma)) {
			dev_kfree_skb(skb);
			goto err_free_rx_bufs;
		}
	...
	g->rx_ring[RX_RING_SIZE - 1].opts1 |= DESC_EOR;
	...
	g->tx_ring[TX_RING_SIZE - 1].opts1 |= DESC_EOR;
...
static void rtl960x_gmac_init_hw(struct rtl960x_gmac *g)
{
	 ...
	 * RX path anyway.
	 */
	regmap_write(g->map8, GMAC_CMD, CMD_RXCHKSUM | CMD_RXJUMBO);
	regmap_write(g->map32, GMAC_TCR, TCR_CONFIG);

	/* CPU tag: parsed into the descriptor opts fields, not inline. */
	regmap_write(g->map32, GMAC_CPUTAGCR,
		     RTL9607C_CPUTAG_CONFIG | CPUTAGCR_EN_RX);
	regmap_write(g->map32, GMAC_CPUTAG1CR, CPUTAG1CR_SID);

	/* Program the RX/TX ring bases. */
	regmap_write(g->map32, GMAC_RXFDP, g->rx_ring_dma);
	regmap_write(g->map16, GMAC_RXCDO, 0);
	regmap_write(g->map8, GMAC_RXRINGSIZE, RX_RING_SIZE - 1);
	regmap_write(g->map8, GMAC_RXCPU_DES_NUM, RX_RING_SIZE - 1);
	regmap_write(g->map8, GMAC_RX_FC_ASSERT_THRES,
		     RX_FC_ASSERT_THRES_VAL);
	regmap_write(g->map8, GMAC_RX_FC_DEASSERT_THRES,
		     RX_FC_DEASSERT_THRES_VAL);

	regmap_write(g->map32, GMAC_TXFDP1, g->tx_ring_dma);
	regmap_write(g->map16, GMAC_TXCDO1, 0);

	rtl960x_gmac_set_hwaddr(g);
	rtl960x_gmac_set_rx_mode(g->dev);

	/* Enable the datapath. */
	regmap_write(g->map32, GMAC_IO_CMD1, RTL9607C_IO_CMD1_CONFIG);
	regmap_write(g->map32, GMAC_IO_CMD, RTL9607C_IO_CMD_CONFIG |
		     IO_CMD_RX_ENABLE | IO_CMD_TX_ENABLE);

	/* Clear and unmask RX + TX-completion interrupts. */
	regmap_write(g->map16, GMAC_ISR, 0xffff);
	regmap_write(g->map32, GMAC_ISR1, 0xffffffff);
	regmap_write(g->map16, GMAC_IMR, INTR_NAPI);
}

static void rtl960x_gmac_mask_irqs(struct rtl960x_gmac *g)
{
	regmap_clear_bits(g->map16, GMAC_IMR, INTR_NAPI);
}

static void rtl960x_gmac_unmask_irqs(struct rtl960x_gmac *g)
{
	regmap_set_bits(g->map16, GMAC_IMR, INTR_NAPI);
}

static int rtl960x_gmac_rx(struct rtl960x_gmac *g, int budget)
{
	struct net_device *dev = g->dev;
	...
	while (done < budget) {
		...
		new_dma = dma_map_single(g->dma_dev, new_skb->data, RX_BUF_SIZE,
					 DMA_FROM_DEVICE);
		if (dma_mapping_error(g->dma_dev, new_dma)) {
		...
		dma_unmap_single(g->dma_dev, g->rx_buf_dma[g->rx_head],
...
static void rtl960x_gmac_tx_reclaim(struct rtl960x_gmac *g)
{
	while (g->tx_tail != g->tx_head) {
		...
		if (skb) {
			dma_unmap_single(g->dma_dev, d->addr, skb->len,
					 DMA_TO_DEVICE);
			g->dev->stats.tx_packets++;
			g->dev->stats.tx_bytes += skb->len;
...
static int rtl960x_gmac_poll(struct napi_struct *napi, int budget)
{
	...
	if (netif_queue_stopped(g->dev) &&
	    (g->tx_head + 1) % TX_RING_SIZE != g->tx_tail)
		netif_wake_queue(g->dev);
...
static irqreturn_t rtl960x_gmac_isr(int irq, void *dev_id)
{
	...
	regmap_read(g->map16, GMAC_ISR, &status);
	status &= INTR_NAPI;

	if (!status)
		return IRQ_NONE;

	regmap_write(g->map16, GMAC_ISR, status);
					   ...
					   struct net_device *dev)
{
	...
	dma = dma_map_single(g->dma_dev, skb->data, len, DMA_TO_DEVICE);
	if (dma_mapping_error(g->dma_dev, dma)) {
		spin_unlock(&g->tx_lock);
	...
	regmap_write_bits(g->map32, GMAC_IO_CMD, IO_CMD_TX_POLL,
			  IO_CMD_TX_POLL);
...
static void rtl960x_gmac_free_tx(struct rtl960x_gmac *g)
{
	...
	for (i = 0; i < TX_RING_SIZE; i++) {
		...
		if (!g->tx_skb[i])
			continue;
		dma_unmap_single(g->dma_dev, d->addr, g->tx_skb[i]->len,
				 DMA_TO_DEVICE);
...
static int rtl960x_gmac_open(struct net_device *dev)
{
	...
	ret = rtl960x_gmac_init_rings(g);
	if (ret)
		return ret;

	ret = request_irq(g->irq, rtl960x_gmac_isr, 0, dev->name, g);
	if (ret) {
				  ...
				  struct ethtool_regs *regs, void *p)
{
	...
	for (i = 0; i < RTL960X_GMAC_REGS_DUMP_LEN / sizeof(u32); i++)
		regmap_read(g->map32, i * sizeof(u32), &buf[i]);
}
...
{
	struct rtl960x_gmac *g = gmac;

	for (int i = 0; i < ARRAY_SIZE(g->dsa_meta); i++) {
		if (!g->dsa_meta[i])
			continue;

		metadata_dst_free(g->dsa_meta[i]);
		g->dsa_meta[i] = NULL;
	}
}

static int rtl960x_gmac_alloc_metadata_dst(struct rtl960x_gmac *g)
{
	for (int i = 0; i < ARRAY_SIZE(g->dsa_meta); i++) {
...
static int rtl960x_gmac_probe(struct platform_device *pdev)
{
	...
	dev = devm_alloc_etherdev(&pdev->dev, sizeof(*g));
	if (!dev)
		return -ENOMEM;
	...
	ret = rtl960x_gmac_alloc_metadata_dst(g);
	if (ret) {
		rtl960x_gmac_free_metadata_dst(g);
		return ret;
	}
	...
	if (IS_ERR(base))
		return PTR_ERR(base);
	g->map8 = devm_regmap_init_mmio(&pdev->dev, base,
					&rtl960x_gmac_regmap8_config);
	if (IS_ERR(g->map8))
		return dev_err_probe(&pdev->dev, PTR_ERR(g->map8),
				     "failed to initialize 8-bit regmap\n");
	g->map16 = devm_regmap_init_mmio(&pdev->dev, base,
					 &rtl960x_gmac_regmap16_config);
	if (IS_ERR(g->map16))
		return dev_err_probe(&pdev->dev, PTR_ERR(g->map16),
				     "failed to initialize 16-bit regmap\n");
	g->map32 = devm_regmap_init_mmio(&pdev->dev, base,
					 &rtl960x_gmac_regmap32_config);
	if (IS_ERR(g->map32))
		return dev_err_probe(&pdev->dev, PTR_ERR(g->map32),
	...
	g->rx_ring = dmam_alloc_coherent(&pdev->dev,
					 RX_RING_SIZE * sizeof(*g->rx_ring),
					 &g->rx_ring_dma, GFP_KERNEL);
	if (!g->rx_ring)
		return -ENOMEM;
	g->tx_ring = dmam_alloc_coherent(&pdev->dev,
					 TX_RING_SIZE * sizeof(*g->tx_ring),
					 &g->tx_ring_dma, GFP_KERNEL);
	if (!g->tx_ring)
		return -ENOMEM;

	g->gmac_rst = devm_reset_control_get_optional_exclusive(&pdev->dev,
								NULL);
	if (IS_ERR(g->gmac_rst))
		return dev_err_probe(&pdev->dev, PTR_ERR(g->gmac_rst),
				     "failed to get gmac reset\n");
	ret = reset_control_deassert(g->gmac_rst);
	if (ret)
		return dev_err_probe(&pdev->dev, ret,
	...
	netif_napi_add(dev, &g->napi, rtl960x_gmac_poll);
	...
	netdev_info(dev, "RTL960x/LUNA GMAC %pR irq %d\n", res, g->irq);
