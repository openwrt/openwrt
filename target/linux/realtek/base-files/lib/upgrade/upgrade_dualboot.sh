platform_check_image_target()
{
	...
	print_error() {
		echo "Invalid image board target ${img_board_target} for this platform: ${board}. Use correct image for this platform"
	}

	case "$img_board_target" in
	plasmacloud_psx8)
		[ "$board" = "plasmacloud,psx8" ] && return 0
		print_error
		return 1
		;;
	plasmacloud_psx10)
		[ "$board" = "plasmacloud,psx10" ] && return 0
		print_error
		return 1
		;;
	*)
