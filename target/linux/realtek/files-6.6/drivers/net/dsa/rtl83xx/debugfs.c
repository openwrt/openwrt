...
static int pmsks_table_show(struct seq_file *m, void *v)
{
	struct rtl838x_switch_priv *priv = m->private;
...
static int pmsks_table_open(struct inode *inode, struct file *filp)
{
	return single_open(filp, pmsks_table_show, inode->i_private);
}

static const struct file_operations pmsks_table_fops = {
	.owner = THIS_MODULE,
	.open = pmsks_table_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int vlan_profiles_show(struct seq_file *m, void *v)
{
	struct rtl838x_switch_priv *priv = m->private;
...
static int vlan_profiles_open(struct inode *inode, struct file *filp)
{
	return single_open(filp, vlan_profiles_show, inode->i_private);
}

static const struct file_operations vlan_profiles_fops = {
	.owner = THIS_MODULE,
	.open = vlan_profiles_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int vlan_table_show(struct seq_file *m, void *v)
{
	struct rtl838x_switch_priv *priv = m->private;
...
static int vlan_table_open(struct inode *inode, struct file *filp)
{
	return single_open(filp, vlan_table_show, inode->i_private);
}

static const struct file_operations vlan_table_fops = {
	.owner = THIS_MODULE,
	.open = vlan_table_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
void rtl838x_dbgfs_init(struct rtl838x_switch_priv *priv)
{
	...
	debugfs_create_file("port_masks_table", 0400, rtl838x_dir, priv, &pmsks_table_fops);

	debugfs_create_file("vlan_profiles", 0400, rtl838x_dir, priv, &vlan_profiles_fops);

	debugfs_create_file("vlan_table", 0400, rtl838x_dir, priv, &vlan_table_fops);
