...
int rtph_821x_match_phy_device(struct phy_device *phydev)
{
	 ...
	 */
	oldpage = phy_port_read_paged(phydev, port, rtph_soc.rawpage, 31);
	phy_port_write_paged(phydev, port, rtph_soc.rawpage, 31, 0xa42);
	phy_port_write_paged(phydev, port, rtph_soc.rawpage, 29, 0x008);
	phy_port_write_paged(phydev, port, rtph_soc.rawpage, 31, 0x278);
	phy_port_write_paged(phydev, port, rtph_soc.rawpage, 18, 0x455);
	phy_port_write_paged(phydev, port, rtph_soc.rawpage, 31, 0x260);
	chip_mode = phy_port_read_paged(phydev, port, rtph_soc.rawpage, 18);
	phy_port_write_paged(phydev, port, rtph_soc.rawpage, 31, 0xa42);
	phy_port_write_paged(phydev, port, rtph_soc.rawpage, 29, 0x000);
	phy_port_write_paged(phydev, port, rtph_soc.rawpage, 31, oldpage);
	...
	return RTPH_PHY_IS_RTL8214FB;
}

static int rtph_8218b_ext_match_phy_device(struct phy_device *phydev)
{
	return rtph_821x_match_phy_device(phydev) == RTPH_PHY_IS_RTL8218B_E;
}

static int rtph_8214fc_match_phy_device(struct phy_device *phydev)
{
	return rtph_821x_match_phy_device(phydev) == RTPH_PHY_IS_RTL8214FC;
}
...
{
	/* fiber ports */
	phy_write_paged(phydev, rtph_soc.rawpage, RTL821XEXT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_FIBRE);
	phy_modify(phydev, 0x10, BMCR_PDOWN, on ? 0 : BMCR_PDOWN);

	/* copper ports */
	phy_write_paged(phydev, rtph_soc.rawpage, RTL821XEXT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_COPPER);
...
void rtl9300_sds_rst(int sds_num, u32 mode)
{
	...
	iomask32(RTPH_930X_SDS_MASK << rtl9300_sds_lsb[sds_num],
		 RTPH_930X_SDS_OFF << rtl9300_sds_lsb[sds_num],
		 RTPH_REG(rtl9300_sds_regs[sds_num]));
	mdelay(10);

	iomask32(RTPH_930X_SDS_MASK << rtl9300_sds_lsb[sds_num], mode << rtl9300_sds_lsb[sds_num],
		 RTPH_REG(rtl9300_sds_regs[sds_num]));
	mdelay(10);
...
static void rtl821x_phy_setup_package_broadcast(struct phy_device *phydev, bool enable)
{
	...
	/* select main page 0 */
	phy_write_paged(phydev, rtph_soc.rawpage, RTL8XXX_PAGE_SELECT, RTL8XXX_PAGE_MAIN);
	/* write to 0x8 to register 0x1d on main page 0 */
	phy_write_paged(phydev, rtph_soc.rawpage, RTL821XINT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_INTERNAL);
	/* select page 0x266 */
	phy_write_paged(phydev, rtph_soc.rawpage, RTL8XXX_PAGE_SELECT, RTL821X_PAGE_PORT);
	/* set phy id and target broadcast bitmap in register 0x16 on page 0x266 */
	phy_write_paged(phydev, rtph_soc.rawpage, 0x16, (enable?0xff00:0x00) | mac);
...
static int rtl8380_configure_int_rtl8218b(struct phy_device *phydev)
{
	...
	/* Ready PHY for patch */
	for (int p = 0; p < 8; p++) {
		phy_package_port_write_paged(phydev, p, rtph_soc.rawpage, RTL8XXX_PAGE_SELECT, RTL821X_PAGE_PATCH);
		phy_package_port_write_paged(phydev, p, rtph_soc.rawpage, 0x10, 0x0010);
	}
	msleep(500);
	for (int p = 0; p < 8; p++) {
		int i;
	...
	for (int p = 0; p < 8; p++) {
		...
		while (rtl838x_6275B_intPhy_perport[i * 2]) {
			phy_package_port_write_paged(phydev, p, rtph_soc.rawpage,
...
static int rtl8380_configure_ext_rtl8218b(struct phy_device *phydev)
{
	...
	/* Get Chip revision */
	phy_write_paged(phydev, rtph_soc.rawpage, RTL8XXX_PAGE_SELECT, RTL8XXX_PAGE_MAIN);
	phy_write_paged(phydev, rtph_soc.rawpage, 0x1b, 0x4);
	val = phy_read_paged(phydev, rtph_soc.rawpage, 0x1c);
	...
	for (int i = 0; rtl8380_rtl8218b_perchip[i * 3] &&
		...
		phy_package_port_write_paged(phydev, rtl8380_rtl8218b_perchip[i * 3],
					     rtph_soc.rawpage, rtl8380_rtl8218b_perchip[i * 3 + 1],
					     rtl8380_rtl8218b_perchip[i * 3 + 2]);
	}

	/* Enable PHY */
	for (int i = 0; i < 8; i++) {
		phy_package_port_write_paged(phydev, i, rtph_soc.rawpage, RTL8XXX_PAGE_SELECT, RTL8XXX_PAGE_MAIN);
		phy_package_port_write_paged(phydev, i, rtph_soc.rawpage, 0x00, 0x1140);
	}
	mdelay(100);

	/* Request patch */
	for (int i = 0; i < 8; i++) {
		phy_package_port_write_paged(phydev, i, rtph_soc.rawpage, RTL8XXX_PAGE_SELECT, RTL821X_PAGE_PATCH);
		phy_package_port_write_paged(phydev, i, rtph_soc.rawpage, 0x10, 0x0010);
	}
	...
	phy_write_paged(phydev, rtph_soc.rawpage, 30, 8);
	...
	for (int i = 0; rtl8218B_6276B_rtl8380_perport[i * 2]; i++) {
		phy_write_paged(phydev, rtph_soc.rawpage, rtl8218B_6276B_rtl8380_perport[i * 2],
...
static bool rtl8214fc_media_is_fibre(struct phy_device *phydev)
{
	...
	phy_package_write_paged(phydev, rtph_soc.rawpage, RTL821XINT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_INTERNAL);
	val = phy_package_read_paged(phydev, RTL821X_PAGE_PORT, reg[mac % 4]);
	phy_package_write_paged(phydev, rtph_soc.rawpage, RTL821XINT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_AUTO);
...
static void rtl8214fc_power_set(struct phy_device *phydev, int port, bool on)
{
	...
	if (port == PORT_FIBRE) {
		...
		phy_write_paged(phydev, rtph_soc.rawpage, RTL821XINT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_FIBRE);
	} else {
		pr_info("%s: Powering %s COPPER (port %d)\n", __func__, state, phydev->mdio.addr);
		phy_write_paged(phydev, rtph_soc.rawpage, RTL821XINT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_COPPER);
	}
...
static int rtl8380_configure_rtl8214fc(struct phy_device *phydev)
{
	...
	/* detect phy version */
	phy_write_paged(phydev, rtph_soc.rawpage, 27, 0x0004);
	val = phy_read_paged(phydev, rtph_soc.rawpage, 28);
	...
	for (int i = 0; rtl8380_rtl8214fc_perchip[i * 3] &&
		...
		if (rtl8380_rtl8214fc_perchip[i * 3 + 1] == 0x13 && page == 0x260) {
			...
			phy_write_paged(phydev, rtph_soc.rawpage,
	...
	for (int i = 0; i < 4; i++) {
		phy_package_port_write_paged(phydev, i, rtph_soc.rawpage, RTL8XXX_PAGE_SELECT, RTL8XXX_PAGE_MAIN);
		phy_package_port_write_paged(phydev, i, rtph_soc.rawpage, RTL821XEXT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_COPPER);
	}
	...
	for (int i = 0; rtl8380_rtl8214fc_perport[i * 2]; i++) {
		phy_write_paged(phydev, rtph_soc.rawpage, rtl8380_rtl8214fc_perport[i * 2],
	...
	for (int i = 0; i < 4; i++) {
		phy_write_paged(phydev, rtph_soc.rawpage, RTL8XXX_PAGE_SELECT, RTL8XXX_PAGE_MAIN);
		phy_write_paged(phydev, rtph_soc.rawpage, RTL821XEXT_MEDIA_PAGE_SELECT, RTL821X_MEDIA_PAGE_AUTO);
	}
...
static int rtl9300_read_status(struct phy_device *phydev)
{
	...
	if (mode == RTPH_930X_SDS_OFF)
		mode = rtl9300_sds_field_r(sds_num, 0x1f, 9, 11, 7);
	if (mode == RTPH_930X_SDS_MODE_10GBASER) { /* 10GR mode */
	...
	if (latch_status) {
		phydev->link = true;
		if (mode == RTPH_930X_SDS_MODE_10GBASER) {
			phydev->speed = SPEED_10000;
...
void rtl9300_force_sds_mode(int sds, phy_interface_t phy_if)
{
	...
	case PHY_INTERFACE_MODE_SGMII:
		sds_mode = RTPH_930X_SDS_MODE_SGMII;
		lc_on = false;
		lc_value = 0x1;
		break;

	case PHY_INTERFACE_MODE_HSGMII:
		sds_mode = RTPH_930X_SDS_MODE_HSGMII;
		lc_value = 0x3;
		/* Configure LC */
		break;

	case PHY_INTERFACE_MODE_1000BASEX:
		sds_mode = RTPH_930X_SDS_MODE_1000BASEX;
		lc_on = false;
		break;

	case PHY_INTERFACE_MODE_2500BASEX:
		sds_mode = RTPH_930X_SDS_MODE_2500BASEX;
		lc_value = 0x3;
		/* Configure LC */
		break;

	case PHY_INTERFACE_MODE_10GBASER:
		sds_mode = RTPH_930X_SDS_MODE_10GBASER;
		lc_on = true;
		lc_value = 0x5;
		break;
	...
	/* SerDes off */
	rtl9300_sds_field_w(sds, 0x1f, 9, 11, 7, RTPH_930X_SDS_OFF);
...
void rtl9300_do_rx_calibration(int sds, phy_interface_t phy_mode)
{
	...
	if (rtl9300_sds_field_r(sds, 0x1f, 9, 11, 7) == RTPH_930X_SDS_MODE_10GBASER) {
...
int rtl9300_serdes_setup(int port, int sds_num, phy_interface_t phy_mode)
{
	...
	/* Turn Off Serdes */
	rtl9300_sds_rst(sds_num, RTPH_930X_SDS_OFF);
