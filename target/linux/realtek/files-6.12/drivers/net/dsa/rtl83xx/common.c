...
static int __init rtl83xx_mdio_probe(struct rtl838x_switch_priv *priv)
{
	...
	for_each_node_by_name(dn, "port") {
		...
		if (pcs_node) {
			priv->ports[pn].swap_rx_polarity = of_property_read_bool(pcs_node, "realtek,pnswap-rx");
			priv->ports[pn].swap_tx_polarity = of_property_read_bool(pcs_node, "realtek,pnswap-tx");
		} else {
