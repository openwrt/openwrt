 ...
 * aligned to a multiple of 4. Current implementation realizes this with a single buffer that
 * can hold the maximum possible data.
 */
...
static void rtl_ecc_wait_for_engine(struct rtl_ecc_ctx *ctx)
{
	...
	ret = regmap_read_poll_timeout(rtlc->regmap, RTL_ECC_STATUS, status,
				       !(status & RTL_ECC_OP_STATUS), 12, 240);
	if (ret) {
		ctx->rc_status = ret;
		return;
	}
...
static int rtl_ecc_finish_io_req(struct nand_device *nand, struct nand_page_io_req *req)
{
	...
	if (unlikely(failure))
		...
		dev_warn(rtlc->dev, "%d bitflip(s) detected\n", ctx->rc_bitflips);
...
static int rtl_ecc_check_support(struct nand_device *nand)
{
	 ...
	 *
	 * It can run for arbitrary NAND flash chips with different block and OOB sizes.
	 * Currently there are only two known devices in the wild that have NAND flash and
	 * make use of this ECC engine (Linksys LGS328C & LGS352C). To keep compatibility
	 * with vendor firmware, new modes can only be added when new data layouts have
	 * been analyzed.
	 *
	 * For now only allow BCH6 on flash with 2048 byte blocks and 64 bytes oob.
	 */
...
{
	struct rtl_ecc_ctx *ctx = nand->ecc.ctx.priv;
...
static int rtl_ecc_probe(struct platform_device *pdev)
{
	...
	/*
	 * Focus on simplicity and stability and use a preallocated DMA buffer for data
	 * exchange with the engine. Invalidating/flushing caches is faster than reading/
	 * writing uncached memory. Make it a noncoherent memory model.
	 */
