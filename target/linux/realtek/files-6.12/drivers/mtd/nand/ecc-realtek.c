 ...
 * be added when new data layouts have been analyzed. For now only allow BCH6 on flash with 2048
 * byte blocks and 64 bytes oob.
 */
...
static int rtl_ecc_probe(struct platform_device *pdev)
{
	...
	/*
	 * Focus on simplicity and stability and use a preallocated DMA buffer for data exchange
	 * with the engine. Make it a noncoherent memory model as invalidating/flushing caches
	 * is faster than reading/writing uncached memory.
	 */
