...
static int alpine_msix_allocate_sgi(struct alpine_msix_data *priv,
                                    int num_req) {
  int first;
  ...
  first =
      bitmap_find_next_zero_area(priv->msi_map, priv->num_spis, 0, num_req, 0);
...
static int alpine_msix_middle_domain_alloc(struct irq_domain *domain,
  ...
  for (i = 0; i < nr_irqs; i++) {
    ...
    irq_domain_set_hwirq_and_chip(domain, virq + i, sgi + i, &middle_irq_chip,
                                  priv);
  }
...
static int alpine_msix_init_domains(struct alpine_msix_data *priv,
  ...
  middle_domain =
      irq_domain_add_tree(NULL, &alpine_msix_middle_domain_ops, priv);
  if (!middle_domain) {
  ...
  msi_domain = pci_msi_create_irq_domain(
      of_node_to_fwnode(node), &alpine_msix_domain_info, middle_domain);
  if (!msi_domain) {
...
int al_msix_init(void) {
  ...
  priv->msi_map = kzalloc(
      sizeof(*priv->msi_map) * BITS_TO_LONGS(priv->num_spis), GFP_KERNEL);
  if (!priv->msi_map) {
    ret = -ENOMEM;
    goto err_priv;
  }
