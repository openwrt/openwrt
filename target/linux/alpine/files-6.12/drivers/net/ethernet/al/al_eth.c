...
static int board_params_debug;
module_param(board_params_debug, int, 0644);
MODULE_PARM_DESC(board_params_debug,
                 "Dump raw/decoded board-params registers written by bootloader");
...
static const char *al_eth_bp_media_type_to_str(u32 media_type) {
  switch (media_type) {
...
static const char *al_eth_bp_phy_if_to_str(u32 phy_if) {
  switch (phy_if) {
  case AL_ETH_BOARD_PHY_IF_MDIO:
    return "MDIO";
  case AL_ETH_BOARD_PHY_IF_XMDIO:
    return "XMDIO";
  case AL_ETH_BOARD_PHY_IF_I2C:
    return "I2C";
  default:
    return "UNKNOWN";
  }
...
static const char *al_eth_bp_mdio_freq_to_str(u32 mdio_freq) {
  switch (mdio_freq) {
...
static void al_eth_dump_dt_board_props(struct al_eth_adapter *adapter,
                                       const char *tag) {
  struct device_node *np = adapter->pdev->dev.of_node;
  u32 v;

  if (!np) {
    dev_info(&adapter->pdev->dev, "%s: no PCI of_node attached\n", tag);
    return;
  }

  dev_info(&adapter->pdev->dev, "%s: PCI of_node=%pOF\n", tag, np);

  if (!of_property_read_u32(np, "al,media-type", &v))
    dev_info(&adapter->pdev->dev, "%s: dt al,media-type=%u(%s)\n", tag, v,
             al_eth_bp_media_type_to_str(v));

  if (!of_property_read_u32(np, "al,serdes-grp", &v))
    dev_info(&adapter->pdev->dev, "%s: dt al,serdes-grp=%u\n", tag, v);

  if (!of_property_read_u32(np, "al,serdes-lane", &v))
    dev_info(&adapter->pdev->dev, "%s: dt al,serdes-lane=%u\n", tag, v);

  if (!of_property_read_u32(np, "al,mdio-freq", &v))
    dev_info(&adapter->pdev->dev, "%s: dt al,mdio-freq=%u(%s)\n", tag, v,
             al_eth_bp_mdio_freq_to_str(v));

  dev_info(&adapter->pdev->dev,
           "%s: dt flags: al,an-disable=%u al,dont-override-serdes=%u\n", tag,
           of_property_read_bool(np, "al,an-disable"),
           of_property_read_bool(np, "al,dont-override-serdes"));
}
...
static void al_eth_dump_board_params_regs(struct al_eth_adapter *adapter,
                                          const char *tag) {
  static const u32 bp_ctx_offsets[] = {0x0,   0x4,   0x8,   0xc,   0x10,  0x14,
                                       0x18,  0x1c,  0x400, 0x404, 0x408, 0x40c,
                                       0x410, 0x414, 0x418, 0x41c};
  u32 reg1;
  u32 reg2;
  int i;

  reg1 = readl(adapter->mac_base + AL_ETH_BOARD_PARAMS_REG1_OFFSET);
  reg2 = readl(adapter->mac_base + AL_ETH_BOARD_PARAMS_REG2_OFFSET);

  dev_info(&adapter->pdev->dev,
           "%s: board-params raw: mac_base=%px reg[0x4]=0x%08x reg[0x404]=0x%08x\n",
           tag, adapter->mac_base, reg1, reg2);

  dev_info(&adapter->pdev->dev,
           "%s: reg1 decode: media=%u(%s) ext_phy=%u phy_addr=%u mdio_freq=%u(%s) phy_if=%u(%s) an_mode=%u sfp=%u serdes_grp=%u serdes_lane=%u ref_clk=%u kr_lt=%u kr_fec=%u\n",
           tag, AL_ETH_BP_FIELD(reg1, AL_ETH_BP_MEDIA_TYPE_MASK,
                                AL_ETH_BP_MEDIA_TYPE_SHIFT),
           al_eth_bp_media_type_to_str(AL_ETH_BP_FIELD(
               reg1, AL_ETH_BP_MEDIA_TYPE_MASK, AL_ETH_BP_MEDIA_TYPE_SHIFT)),
           !!(reg1 & AL_ETH_BP_EXT_PHY_BIT),
           AL_ETH_BP_FIELD(reg1, AL_ETH_BP_PHY_ADDR_MASK,
                           AL_ETH_BP_PHY_ADDR_SHIFT),
           AL_ETH_BP_FIELD(reg1, AL_ETH_BP_MDIO_FREQ_MASK,
                           AL_ETH_BP_MDIO_FREQ_SHIFT),
           al_eth_bp_mdio_freq_to_str(AL_ETH_BP_FIELD(
               reg1, AL_ETH_BP_MDIO_FREQ_MASK, AL_ETH_BP_MDIO_FREQ_SHIFT)),
           AL_ETH_BP_FIELD(reg1, AL_ETH_BP_EXT_PHY_IF_MASK,
                           AL_ETH_BP_EXT_PHY_IF_SHIFT),
           al_eth_bp_phy_if_to_str(AL_ETH_BP_FIELD(
               reg1, AL_ETH_BP_EXT_PHY_IF_MASK, AL_ETH_BP_EXT_PHY_IF_SHIFT)),
           !!(reg1 & AL_ETH_BP_AN_MODE_BIT), !!(reg1 & AL_ETH_BP_SFP_EXIST_BIT),
           AL_ETH_BP_FIELD(reg1, AL_ETH_BP_SERDES_GRP_MASK,
                           AL_ETH_BP_SERDES_GRP_SHIFT),
           AL_ETH_BP_FIELD(reg1, AL_ETH_BP_SERDES_LANE_MASK,
                           AL_ETH_BP_SERDES_LANE_SHIFT),
           AL_ETH_BP_FIELD(reg1, AL_ETH_BP_REF_CLK_FREQ_MASK,
                           AL_ETH_BP_REF_CLK_FREQ_SHIFT),
           !!(reg1 & AL_ETH_BP_KR_LT_ENABLE_BIT),
           !!(reg1 & AL_ETH_BP_KR_FEC_ENABLE_BIT));

  dev_info(&adapter->pdev->dev,
           "%s: reg2 decode: dont_override_serdes=%u force_1000base_x=%u an_disable=%u speed=%u half_duplex=%u fc_disable=%u retimer_exist=%u retimer_bus=%u retimer_i2c=0x%x retimer_ch=%u dac=%u dac_len=%u\n",
           tag, !!(reg2 & AL_ETH_BP_DONT_OVERRIDE_SERDES_BIT),
           !!(reg2 & AL_ETH_BP_FORCE_1000_BASE_X_BIT),
           !!(reg2 & AL_ETH_BP_1G_AN_DISABLE_BIT),
           AL_ETH_BP_FIELD(reg2, AL_ETH_BP_1G_SPEED_MASK,
                           AL_ETH_BP_1G_SPEED_SHIFT),
           !!(reg2 & AL_ETH_BP_1G_HALF_DUPLEX_BIT),
           !!(reg2 & AL_ETH_BP_1G_FC_DISABLE_BIT),
           !!(reg2 & AL_ETH_BP_RETIMER_EXIST_BIT),
           AL_ETH_BP_FIELD(reg2, AL_ETH_BP_RETIMER_BUS_ID_MASK,
                           AL_ETH_BP_RETIMER_BUS_ID_SHIFT),
           AL_ETH_BP_FIELD(reg2, AL_ETH_BP_RETIMER_I2C_ADDR_MASK,
                           AL_ETH_BP_RETIMER_I2C_ADDR_SHIFT),
           !!(reg2 & AL_ETH_BP_RETIMER_CHANNEL_BIT),
           !!(reg2 & AL_ETH_BP_DAC_BIT),
           AL_ETH_BP_FIELD(reg2, AL_ETH_BP_DAC_LENGTH_MASK,
                           AL_ETH_BP_DAC_LENGTH_SHIFT));

  if (!board_params_debug)
    return;

  for (i = 0; i < ARRAY_SIZE(bp_ctx_offsets); i++) {
    u32 off = bp_ctx_offsets[i];
    u32 val = readl(adapter->mac_base + off);

    dev_info(&adapter->pdev->dev, "%s: mac[0x%03x]=0x%08x\n", tag, off, val);
  }
}
...
static int al_eth_sw_mdio_probe(struct al_eth_adapter *adapter) {
  ...
  if (board_params_debug) {
    if (dev->of_node)
      dev_info(dev, "sw_mdio_probe: PCI of_node=%pOF\n", dev->of_node);
    else
      dev_info(dev, "sw_mdio_probe: PCI of_node is NULL\n");
  }
...
static int al_eth_board_params_init(struct al_eth_adapter *adapter) {
  if (adapter->board_type == ALPINE_NIC) {
    ...
    if (board_params_debug) {
      al_eth_dump_dt_board_props(adapter, "board_params_pre_get");
      al_eth_dump_board_params_regs(adapter, "board_params_pre_get");
    }
    ...
    if (rc) {
      ...
      al_eth_dump_dt_board_props(adapter, "board_params_get_failed");
      al_eth_dump_board_params_regs(adapter, "board_params_get_failed");
      ...
      if (board_params_debug) {
        al_eth_dump_dt_board_props(adapter, "board_params_after_dt_fallback");
        al_eth_dump_board_params_regs(adapter, "board_params_after_dt_fallback");
      }
    }
