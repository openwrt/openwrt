			 ...
			 struct netlink_ext_ack *extack)
{
	...
	if (allocated && gsw->conduit)
		vlan_vid_add(gsw->conduit, htons(ETH_P_8021Q), vlan->vid);
...
static void mt7620_gsw_sync_phy_state(struct dsa_switch *ds)
{
	struct dsa_port *dp;
				...
				struct dsa_switch *ds)
{
	...
	phy_polling |= PHY_AN_EN | PHY_PRE_EN | PMY_MDC_CONF(5) |
		       PHY_EMB_AN_EN |
		       FIELD_PREP(PHY_END_ADDR, gsw->ephy_base + 5) |
