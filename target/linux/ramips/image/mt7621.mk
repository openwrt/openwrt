...
define Build/dna-bootfs
	...
	mkdir -p ${temp_dir}/my_root/boot

	# populate the boot fs with the dtb and with either initramfs kernel or
	# the normal kernel
	$(CP) $(KDIR)/image-$(firstword $(DEVICE_DTS)).dtb ${temp_dir}/my_root/boot/dtb

	$(if $(findstring with-initrd,$(word 1,$(1))),\
		( \
			$(CP) $@ ${temp_dir}/my_root/boot/uImage \
		) , \
		( \
			$(CP) $(IMAGE_KERNEL) ${temp_dir}/my_root/boot/uImage \
		) \
	)

	# create ubifs
	$(STAGING_DIR_HOST)/bin/mkfs.ubifs ${MKUBIFS_OPTS} -r ${temp_dir}/my_root -o $@.new
	rm -rf ${temp_dir}
	mv $@.new $@
endef
